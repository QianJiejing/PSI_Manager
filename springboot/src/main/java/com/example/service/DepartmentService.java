...
public class DepartmentService {
    ...
    public List<Department> selectTree() {
        ...
        for (Department level1 : level1DepartmentList) {
            ...
            List<Department> level2DepartmentList = this.selectAll(params1);
            level1.setChildren(level2DepartmentList);  // 设置一级节点的子节点
            for (Department level2 : level2DepartmentList) {
                Integer level2Id = level2.getId();
                Department params2 = new Department();
                params2.setPid(level2Id);  // 查询三级节点
                List<Department> level3DepartmentList = this.selectAll(params2);
                level2.setChildren(level3DepartmentList);   // 设置儿级节点的子节点
            }
        }
